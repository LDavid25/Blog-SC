...
abstract class Validador{
    ...
    protected function validar_titulo($conexion, $titulo) {
        if (!$this->variable_iniciada($titulo)) {
            return "Debes escribír un título";
        } else {
            $this->titulo = $titulo;
        }

        if (strlen($titulo) > 255) {
            return "El título no puede pasar de 255 caracteres";
        }
        if (RepositorioEntradas :: titulo_existe($conexion, $titulo)) {
            return "Ya exíste una entrada con ese título, intente con otro";
        }
    }

    protected function validar_url($conexion, $url) {
        if (!$this->variable_iniciada($url)) {
            return "Debes escribir un URL";
        } else {
            $this->url = $url;
        }
